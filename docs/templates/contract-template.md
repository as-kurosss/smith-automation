...
**🔸 Guarantees (AFTER):** If `Ok`: [what changes]; If `Err`: [what DOES NOT change — state unchanged per Канон 10.2]
...
- [ ] Input validation happens BEFORE any backend call (Канон 10.1)
...
- [ ] COM / blocking calls isolated in `spawn_blocking` (Канон 5.3)
